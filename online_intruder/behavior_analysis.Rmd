...
```{r plreg_not_named}
not_named <- c("kite", "rightKite", "rustedHinge", "hinge", "random")
named <- setdiff(levelsInOrder, not_named)
names <-
pl.restricted <-
  base_d %>%
  group_by(ID) %>%
  ...
  inner_join(symbolic, by=c("reference", "intruder")) %>%
  inner_join(cornet, by=c("reference", "intruder")) %>%
  filter(reference %in% not_named) %>%
  filter(intruder %in% not_named) %>%
  group_modify(function(d,k) {
    d %>%
      mutate(humans = scale(humans),
             symbolic = scale(symbolic),
             cornet = scale(cornet)) %>%
      lm(humans ~ 0 + symbolic + cornet, data=.) %>%
      tidy
  }) %>%
...
ggsave("/tmp/restricted.svg", pl.restricted, height=3, width=3)
```
