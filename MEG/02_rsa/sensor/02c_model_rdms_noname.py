...
ordered = ["kite", "rightKite", "rustedHinge", "hinge", "random"]
...
rdms_movie = rdms_movie.subset_pattern("shape", ordered)
...
for i in range(1,-1,-1):
    ...
    for x in np.where(p_values < .05)[0]:
        ...
        p05[filt] = mu[filt]
        print(f"Model {model_names[i]}; from {np.min((np.array(filt)-offset) * .004)*1000}ms to {np.max((np.array(filt)-offset) * .004)*1000}ms; max at {times[np.where(mu == np.max(mu))[0][0]]}, p is {p_values[x]}")
...
pretty_plot()
plt.savefig(f"./figs/lm_{cm}_{method}_{smooth}_noname.svg")
