...
def main(sub, method="crossnobis"):
    # Prepare beta maps
    glm = pickle.load(open(f"{base}nilearn/{sub}/func/{sub}_task-category_model-spm_full-false.pkl", "rb"))
    ref_img = load_img(f"{base}/fmriprep/{sub}/func/{sub}_task-category_run-01_space-MNI152NLin2009cAsym_res-2_desc-preproc_bold.nii.gz")
    ...
    conditions = list(glm.design_matrices_[0].columns[:9])
    ...
    def map_one(chunks):
        ...
        rdm = calc_rdm(center_data, method=method, descriptor="condition", cv_descriptor="run")
        return rdm
    ...
    pickle.dump(SL_RDM, open(f"{base}rsa/{sub}/func/{sub}_task-category_method-{method}_RDM.pkl", "wb"))
